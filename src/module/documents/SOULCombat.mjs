 ...
 */
export default class SOULCombat extends foundry.documents.Combat {
  /** @inheritdoc */
  prepareDerivedData() {
     ...
     * @param {SOULCombat} combat      The combat preparing derived data.
     */
    Hooks.callAll("SOUL.prepareCombatData", this);
  }
   ...
   * @returns {Promise<import("./SOULCombatant.mjs").default>} The created Combatant.
   */
  async addPlayer() {
    const data = {
      type: "player",
    ...
    const fdObject = await foundry.applications.api.DialogV2.input({
      window: { title: "SOUL.Combat.AddPlayerCombatTracker" },
   ...
   * @returns {Promise<SOULCombat|null>}   A Promise which resolves to the created Document, or null if the dialog was
    *                                     closed.
   */
